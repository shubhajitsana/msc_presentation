...
\begin{frame}[t]{Formalism $\&$ Approach}
	...
	\begin{itemize}
		...
		% $$
		\begin{gathered}
			\mathcal{B}(\mathrm{B}^{+} \rightarrow \bar{\mathrm{D}}^{0}\pi^{+}\pi^{-}\pi^{+}) = \frac{N_{sig}}{2 \cdot \varepsilon \cdot \mathscr{L} \cdot \sigma_{B^+B^-} \cdot \mathcal{B}(\bar{\rm D}^0 \rightarrow K^{+} \pi^{-})}
		\end{gathered}
		\item Data Set: MC14ri$\_$a inclusive MC (100 $fb^{-1}$)
		...
		\begin{itemize}
			...
			\item Polar angle $\to$ 0 < $\theta$ < 126.87: cosTheta >= -0.6 
			\end{scriptsize}
		\end{itemize}
