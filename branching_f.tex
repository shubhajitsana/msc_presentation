...
%--------------------------------------------------------------------------
\begin{frame}[t]{Formalism $\&$ Approach}
	\begin{scriptsize}
	\begin{itemize}
		...
		% $$
		% \begin{gathered}
		% 	\mathcal{B} = \dfrac{N_{selected}}{2 f_{\pm} N_{B\bar{B}}\epsilon}\\
		% \end{gathered}
		% $$
		% \begin{equation}
        $$
			\mathcal{B}(\mathrm{B}^{+} \rightarrow \bar{\mathrm{D}}^{0}\pi^{+}\pi^{-}\pi^{+}) = \frac{N_{s i g}}{2 \cdot \varepsilon \cdot \mathscr{L} \cdot \sigma_{B^+B^-} \cdot \mathcal{B}(\bar{\rm D}^0 \rightarrow K^{+} \pi^{-})}
        $$
		% \end{equation}
		\item Data Set: MC15ri$\_$a inclusive MC (100 $fb^{-1}$)
		\item \textbf{Track selection:}
		\begin{itemize}
			\begin{scriptsize}
			\item Transverse impact parameter $\vert$d0$\vert$ < 0.2 cm.
			\item Longitudinal impact parameter $\vert$z0$\vert$ < 1 cm.
			\item Polar angle $\rightarrow$ 0 < $\theta$ < 126.87: $\cos\theta$ >= -0.6 
			\end{scriptsize}
		\end{itemize}
...
\begin{scriptsize}
		...
		\begin{figure}
			...
			\caption{Plotting M$_{bc}$ for different set}
		\end{figure}
		
		\column{0.5\textwidth}
		\begin{figure}
			\includegraphics[height = 5 cm, width = 6cm]{Mbc_sig.png}
			\caption{Plotting M$_{bc}$ for signal}
		\end{figure}
